...
- Core routes: `/login`, `/tickets`, `/tickets/new` (schema-driven), `/tickets/[id]`, `/notifications`, `/inbox` (actionable queue), `/dashboard`, `/assistant`; **`/handbook` redirects to `/assistant`** (bookmarks preserved)
...
- **Ticket list / feed UX:** ticket feed supports **search**, **date filter**, **department / class / topic-category / location / tag** filters, and **Active vs Completed** (`statusGroup`) via `TicketFeedLayout` + `useTicketListQuery` / `buildListParams`. **`tagId`** filter: `GET /api/tickets/tags` returns tags visible on the actor’s tickets (for ComboBox options); list query accepts **`tagId`**. URL sync on `/tickets` includes `tagId` where used. **`buildTicketsFeedHref`** (`lib/tickets-deep-link.ts`) builds dashboard → `/tickets` links (includes **`tagId`** among deep-link keys). Keep column widths consistent with `FEED_COL_WIDTHS` in `lib/polish.ts` when changing layouts.
- **Admin ticket export (feed footer):** **`TicketExportMenu`** next to pagination on `/tickets` — **Export** opens a small menu; **Download CSV** (only option in UI today) uses the **same filtered query** as the feed (full result set, batched on server, not current page only) via **`GET /api/tickets/export/csv`**. **ADMIN-only**, UTF‑8 CSV with BOM, **25k row cap**, same query params as ticket list. **API-only** (no web button yet): **`GET /api/tickets/export`** → Excel (`.xlsx`, **exceljs**); optional **Google Sheets** — **`POST /api/tickets/export/google-sheet`**, **`GET /api/tickets/export/google-sheet/status`** → `{ enabled }`. Env for Sheets: **`GOOGLE_TICKETS_EXPORT_SERVICE_ACCOUNT_JSON`** or **`GOOGLE_TICKETS_EXPORT_SERVICE_ACCOUNT_PATH`** (+ Sheets + Drive API on the key); listed in **`turbo.json` `globalEnv`**. **`FeedPaginationBar`** accepts optional **`trailing`** slot for the export control; menu is **in-document** (`absolute` left of button) to avoid **CSS zoom** vs portaled `fixed` misalignment.
...
- **Stage 23 (User Visibility & Inbox):** Admin can set a studio user’s default location and add/remove additional locations (Admin → Users → Locations). Studio users see a location filter on the portal when they have multiple allowed studios; GET /tickets returns 403 if a STUDIO_USER filters by a studio outside their allowed set. Department users see inbox topic folders (All + support topics) with active counts (grouped count by supportTopicId). Studio users cannot create/update subtasks or transition ticket status; they can view progress, comment (non-internal), and receive notifications. Actionable nav remains visible for DEPARTMENT_USER and ADMIN. **Portal** “My tickets” / “By studio(s)” feeds can also filter by **`tagId`** (same API as staff feed).
- **Dashboard (`GET /dashboard/summary`):** `DashboardService` applies `TicketVisibilityService` filters. **ADMIN / DEPARTMENT_USER:** counts for NEW / IN_PROGRESS / RESOLVED+CLOSED, avg completion hours (last 30d), `supportByType` (support topic breakdown), `maintenanceByLocation` (maintenance tickets by studio). **STUDIO_USER:** open vs completed, same avg window, `byLocation` (optional `?studioId` when within allowed scope). UI: `/dashboard` and portal **Dashboard** tab reuse `dashboardApi.summary`. **Bar chart rows** can deep-link to **`/tickets`** with matching filters + optional KPI date range via **`buildTicketsFeedHref`**.
...
---
*Last updated: April 2026 — Plus: **`/handbook` → `/assistant`** redirect; ticket feed **tag** filter + **`GET /tickets/tags`**; **admin export**: CSV from **`TicketExportMenu`** on `/tickets`; Excel + optional **Google Sheets** via API (`exceljs`, export routes, optional `GOOGLE_TICKETS_EXPORT_*` / `turbo.json`); **`FeedPaginationBar` `trailing`**; dashboard **→ `/tickets` deep links** (`buildTicketsFeedHref`, `tagId`); portal tag filter parity; Claude Code vs Cursor (§0); Lease IQ lease-source list metadata + migration `20260403180000_lease_source_list_meta`; ongoing polish on admin/dispatch/inbox/portal/ticket surfaces.*
