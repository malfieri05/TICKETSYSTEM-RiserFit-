...
export class SlaService {
  ...
  compute(ticket: {
    ...
    const targetHours = SLA_RESOLUTION_HOURS[ticket.priority] ?? SLA_RESOLUTION_HOURS['MEDIUM'];

    const referenceDate = isResolved && ticket.resolvedAt
      ? ticket.resolvedAt
      : new Date();
    ...
    const percentUsed = Math.min(100, parseFloat(((elapsedHours / targetHours) * 100).toFixed(1)));

    if (isResolved) {
      return { status: 'RESOLVED', targetHours, elapsedHours, remainingHours, percentUsed };
    }

    if (elapsedHours >= targetHours) {
      return { status: 'BREACHED', targetHours, elapsedHours, remainingHours, percentUsed };
    }

    if (remainingHours / targetHours < AT_RISK_THRESHOLD) {
      return { status: 'AT_RISK', targetHours, elapsedHours, remainingHours, percentUsed };
    }

    return { status: 'OK', targetHours, elapsedHours, remainingHours, percentUsed };
  }

  /** Annotate a collection of ticket objects with their SLA status. */
  annotateMany<T extends { priority: string; status: string; createdAt: Date; resolvedAt?: Date | null }>(
    tickets: T[],
  ): (T & { sla: SlaStatus })[] {
  ...
  isBreached(ticket: { priority: string; status: string; createdAt: Date }): boolean {
