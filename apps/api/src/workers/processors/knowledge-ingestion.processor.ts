...
import { QUEUES, KnowledgeIngestionJobData } from '../../common/queue/queue.constants';
...
export class KnowledgeIngestionProcessor extends WorkerHost {
  ...
  async process(job: Job<KnowledgeIngestionJobData, void, string>): Promise<void> {
    const { documentId } = job.data;
    try {
      await this.ingestionService.runIngestionForDocument(documentId);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      this.logger.warn(`Knowledge ingestion failed for documentId=${documentId}: ${message}`);
      throw err;
    }
  }
}
